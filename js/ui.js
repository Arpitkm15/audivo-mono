...
import { isCustomAlbumId, resolveCustomAlbum } from './customAlbums.js';
import {
...
const SPOTIFY_HOME_PLAYLIST_ID = '37i9dQZEVXbLZ52XmnySJg';
const HOME_TRENDING_PAGE_SIZE = 10;
const HOME_POPULAR_ARTISTS_PAGE_SIZE = 8;
const HOME_TRENDING_SONGS = [
    'Banjaare - Bairan',
    'Shashwat Sachdev,Jasmine Sandlas,Satinder Sartaaj - Jaiye Sajana',
    'Shashwat Sachdev,Arijit Singh,Irshad Kamil,Armaan Khan - Gehra Hua',
    'Mitta Ror,Swara Verma - Sheesha - Aakhya Mai Aakh Ghali Jo Bairan',
    'Navjot Ahuja - Khat',
    'Paresh Pahuja,Shiv Tandan,Meghdeep Bose - Dooron Dooron',
    'Aditya Rikhari - Sahiba',
    'Arijit Singh,White Noise Collectives,Amitabh Bhattacharya - Sitaare (From "Ikkis")',
    'Shashwat Sachdev,Bombay Rockers,Irshad Kamil,Khan Saab,Reble,Token,Jasmine Sandlas,Sudhir Yaduvanshi - Dhurandhar The Revenge - Aari Aari',
    'Sachin-Jigar,Arijit Singh,Amitabh Bhattacharya - Apna Bana Le',
    'Jawad Ahmad,Shaarib Toshi,Arijit Singh,Shreya Ghoshal - Samjhawan',
    'Kushagra,Bharath,Saaheal - Finding Her',
    'Sai Abhyankkar,Shruti Haasan,Vivek - Pavazha Malli - From "Think Indie"',
    'Karan Aujla,Ikky - Boyfriend',
    'Anuv Jain - Arz Kiya Hai | Coke Studio Bharat',
    'Shashwat Sachdev,Khan Saab,Nusrat Fateh Ali Khan - Jaan Se Guzarte Hain',
    'Shashwat Sachdev,Madhubanti Bagchi,Jasmine Sandlas - Shararat',
    'Gajendra Verma - Mann Mera',
    ...
    'Zeeshan Ali,Yuvraj Tung,Sandeep Aulakh,Honey Dhillon - Ishqa Ve',
    'Virat,Pradeep Solanki,Heena - Bairi',
    'Shashwat Sachdev,Afsana Khan,Irshad Kamil,Reble - Naal Nachna',
    'Mithoon,Arijit Singh,Sunidhi Chauhan,Sayeed Quadri - Darkhaast',
    'Vishal Mishra,Hansika Pareek,Raj Shekhar - Tum Ho Toh (From "Saiyaara")',
    'Shashwat Sachdev,Hanumankind,Jasmine Sandlas,Sudhir Yaduvanshi,Charanjit Ahuja,Muhammad Sadiq,Ranjit Kaur,Babu Singh Maan - Dhurandhar - Title Track',
    'Atif Aslam,Shreya Ghoshal,Sachin Gupta,Sameer Anjaan - Tere Liye',
    'A.R. Rahman,Irshad Kamil - Deewaana Deewaana',
    'Sachet-Parampara,Parampara Tandon,Kausar Munir - Raanjhan (From "Do Patti")',
    'Shashwat Sachdev,Arijit Singh,Irshad Kamil - Phir Se',
    'Anuv Jain - Jo Tum Mere Ho',
    'A.R. Rahman,Chinmayi,Murtuza Khan,Qadir Khan - Tere Bina',
    'Alka Yagnik,Arijit Singh - Agar Tum Saath Ho (From "Tamasha")',
    'Faheem Abdullah,Duha Shah,Vaibhav Pani,Hyder Dar - Ehsaas',
    'Shashwat Sachdev,Simran Choudhary - Lutt Le Gaya',
    'Kailash Kher,Paresh Kamath,Naresh Kamath - Teri Deewani',
    'Pritam,Javed Bashir,Rajat Arora - Ye Tune Kya Kiya',
    'The Rish,Jubin Nautiyal - Barbaad (From "Saiyaara")',
    'Talwiinder,NDS,Rippy Grewal - Haseen',
    'Shashwat Sachdev,Reble,Asha Bhosle,R. D. Burman,Majrooh Sultanpuri - Run Down The City - Monica',
    'thiarajxtt,Bir - Jogi',
    'Arijit Singh,Chinmayi - Mast Magan (From "2 States)',
    'Arijit Singh,Harshdeep Kaur - Zaalima',
    'Gajendra Verma - Mann Mera - Original Version',
    'Dhanda Nyoliwala - Not Guilty',
    'G. V. Prakash,Ken Karunaas - Mutta Kalakki (From "Youth")',
    'AP Dhillon,Shreya Ghoshal - Thodi Si Daaru',
    'Shashwat Sachdev,Shahzad Ali,Subhadeep Das Chowdhury,Armaan Khan,Irshad Kamil,Roshan,Sahir Ludhianvi - Ishq Jalakar - Karvaan',
    'Sheheryar Rehan,Zoha Waseem - Majboor'
];
const HOME_POPULAR_ARTISTS = [
    'Arijit Singh',
    'Pritam',
    'Shreya Ghoshal',
    'Anirudh Ravichander',
...
const TOP_TRENDING_SEARCH_SEEDS = [
    ['Bairan', 'Banjaare'],
...
const parseTrendingSongSeed = (entry) => {
    ...
    const artistsPart = rawEntry.slice(0, separatorIndex).trim();
    const title = rawEntry.slice(separatorIndex + 3).trim();
...
export class UIRenderer {
    ...
    constructor(api, player) {
        ...
        this.searchSuggestionAbortController = null;
        this.searchSuggestionCoverByQuery = new Map();
        ...
        this.spotifyHomeArtistsCache = null;
        this.spotifyHomeTrendingTracksCache = null;
        this.popularArtistsCache = null;
        this.popularArtistsPage = 0;
        this.spotifyTrendingSongsPage = 0;
        this.curatedTrendingSearchSeedsCache = null;
    ...
    createPlaylistCardHTML(playlist, forceCompact = false) {
        const imageId = playlist.squareImage || playlist.image || playlist.uuid;
        const isCompact = forceCompact || cardSettings.isCompactAlbum();
    ...
    createMixCardHTML(mix, forceCompact = false) {
    ...
    createUserPlaylistCardHTML(playlist, customSubtitle = null, forceCompact = false) {
        let imageHTML = '';
        if (playlist.cover) {
    ...
    createAlbumCardHTML(album, forceCompact = false) {
    ...
    createPopularArtistFallbackCardHTML(artist) {
        const isCompact = cardSettings.isCompactArtist();
        const initials = String(artist.name || '')
            .split(/\s+/)
        ...
        return this.createBaseCardHTML({
            type: 'artist',
            id: artist.id,
            href: `/search/${encodeURIComponent(artist.searchQuery || artist.name)}`,
            title: escapeHtml(artist.name),
            subtitle: 'Popular artist',
            imageHTML: `<div class="card-image popular-artist-avatar">${escapeHtml(initials)}</div>`,
            actionButtonsHTML: '',
            isCompact,
            extraClasses: 'popular-artist-card',
        });
    }
    ...
    async updateFullscreenMetadata(track, nextTrack) {
        ...
        if (nextTrack) {
            ...
            const nextTrackTitle = String(nextTrack.title || '')
                .replace(/\s*\(single\)$/i, '')
                .replace(/\s*[\-–—]\s*single$/i, '')
                .trim();
            nextTrackEl.querySelector('.value').textContent = nextTrackTitle || 'Unknown Title';
        } else {
    ...
    async showFullscreenCover(track, nextTrack, lyricsManager, activeElement) {
        ...
        if (canRenderLyrics) {
            ...
            const reusedBackgroundRender = lyricsManager.consumeBackgroundLyricsRender(track.id, lyricsContent);
            if (!reusedBackgroundRender) {
                lyricsContent.innerHTML = '<div class="fullscreen-lyrics-loading">Preparing lyrics...</div>';
                void lyricsManager
                    .preRenderLyricsInBackground(track, activeElement)
                    .then(() => {
                        const currentLyricsContent = document.getElementById('fullscreen-lyrics-content');
                        if (!currentLyricsContent || window.location.hash !== '#fullscreen') return;

                        if (lyricsManager.consumeBackgroundLyricsRender(track.id, currentLyricsContent)) {
                            this.updateFullscreenLyricsVisibility(overlay);
                        }
                    })
                    .catch((error) => console.warn('Fullscreen lyrics background render failed:', error));
            }
        } else {
    ...
    setupFullscreenControls() {
        ...
        // Handle loading state for fullscreen button
        const showLoadingState = () => {
            playBtn.innerHTML = `<div style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;">${SVG_ANIMATE_SPIN(32)}</div>`;
            lastPausedState = null; // Reset state to force update
        };

        const activeElementEventListeners = () => {
            const activeEl = this.player.activeElement;
            if (!activeEl) return;

            activeEl.addEventListener('loadstart', showLoadingState);
            activeEl.addEventListener('play', updatePlayBtn);
            activeEl.addEventListener('pause', updatePlayBtn);
            activeEl.addEventListener('playing', updatePlayBtn);
        };

        activeElementEventListeners();
        updatePlayBtn(); // Set initial state
    ...
    async showPage(pageId) {
        ...
        if (pageId === 'settings') {
            ...
            const savedTab = document.querySelector(`.settings-tab[data-tab="${savedTabName}"]:not(:disabled)`);
            if (savedTab) {
                ...
                const firstEnabledTab = document.querySelector('.settings-tab:not(:disabled)');
                if (firstEnabledTab) {
                    document.querySelectorAll('.settings-tab').forEach((t) => t.classList.remove('active'));
                    document.querySelectorAll('.settings-tab-content').forEach((c) => c.classList.remove('active'));
                    firstEnabledTab.classList.add('active');
                    document.getElementById(`settings-tab-${firstEnabledTab.dataset.tab}`)?.classList.add('active');
                }
            }
        } else {
            document.querySelectorAll('.settings-tab').forEach((t) => t.classList.remove('active'));
    ...
    async renderHomePage() {
        ...
        try {
            ...
            // Editor's Picks removed from Home view.
            ...
            if (welcomeEl) welcomeEl.style.display = hasActivity ? 'none' : 'block';
            if (contentEl) contentEl.style.display = 'block';

            const prevSongsBtn = document.getElementById('prev-songs-btn');
            const nextSongsBtn = document.getElementById('next-songs-btn');
            const prevTrendingSongsBtn = document.getElementById('prev-trending-songs-btn');
            const nextTrendingSongsBtn = document.getElementById('next-trending-songs-btn');
            const prevRecentBtn = document.getElementById('prev-recent-btn');
            const nextRecentBtn = document.getElementById('next-recent-btn');
            const prevAlbumsBtn = document.getElementById('prev-albums-btn');
            const nextAlbumsBtn = document.getElementById('next-albums-btn');
            const prevArtistsBtn = document.getElementById('prev-artists-btn');
            const nextArtistsBtn = document.getElementById('next-artists-btn');
            const clearRecentBtn = document.getElementById('clear-recent-btn');

            if (prevSongsBtn) prevSongsBtn.onclick = () => this.scrollHomeRail('home-recommended-songs', 'prev');
            if (nextSongsBtn) nextSongsBtn.onclick = () => this.scrollHomeRail('home-recommended-songs');
            if (prevTrendingSongsBtn)
                prevTrendingSongsBtn.onclick = () => this.scrollHomeRail('home-trending-songs', 'prev');
            if (nextTrendingSongsBtn) nextTrendingSongsBtn.onclick = () => this.scrollHomeRail('home-trending-songs');
            if (prevRecentBtn) prevRecentBtn.onclick = () => this.scrollHomeRail('home-recent-mixed', 'prev');
            if (nextRecentBtn) nextRecentBtn.onclick = () => this.scrollHomeRail('home-recent-mixed');
            if (prevAlbumsBtn) prevAlbumsBtn.onclick = () => this.scrollHomeRail('home-recommended-albums', 'prev');
            if (nextAlbumsBtn) nextAlbumsBtn.onclick = () => this.scrollHomeRail('home-recommended-albums');
            if (prevArtistsBtn) prevArtistsBtn.onclick = () => this.scrollHomeRail('home-recommended-artists', 'prev');
            if (nextArtistsBtn) nextArtistsBtn.onclick = () => this.scrollHomeRail('home-recommended-artists');
            if (clearRecentBtn)
            ...
            await Promise.all([
                this.renderHomeTrendingSongs(false),
    ...
    async renderHomeSongs(forceRefresh = false, providedSeeds = null) {
        ...
        if (songsContainer) {
            songsContainer.classList.add('home-song-rail');
            songsContainer.classList.add('card-grid');
        }

        if (songsContainer) {
            if (forceRefresh || songsContainer.children.length === 0) {
                this.setHomeRailLoadingState(songsContainer);
            ...
            try {
                ...
                if (filteredTracks.length > 0) {
                    songsContainer.innerHTML = filteredTracks.map((track) => this.createTrackCardHTML(track)).join('');
                    songsContainer.scrollLeft = 0;
                    for (const track of filteredTracks) {
                        const el = songsContainer.querySelector(`[data-track-id="${track.id}"]`);
                        if (el) {
                            trackDataStore.set(el, track);
                            await this.updateLikeState(el, track.type === 'video' ? 'video' : 'track', track.id);
                        }
                    }
                } else {
                    songsContainer.innerHTML = createPlaceholder(
                        'Tell us more about what you like so we can recommend songs!'
                    );
                }
            } catch (e) {
                console.error(e);
    ...
    async renderHomeTrendingSongs(forceRefresh = false) {
        const songsContainer = document.getElementById('home-trending-songs');
        const section = songsContainer?.closest('.content-section');
        ...
        if (forceRefresh || songsContainer.children.length === 0) {
            songsContainer.innerHTML = this.createSkeletonCards(6);
        } else if (!songsContainer.querySelector('.skeleton')) {
            return;
        }

        try {
            const trendingTracks = await this.getSpotifyTrendingTracks(forceRefresh);
            if (trendingTracks.length > 0) {
                songsContainer.innerHTML = trendingTracks.map((track) => this.createTrackCardHTML(track)).join('');
                songsContainer.scrollLeft = 0;
                ...
                songsContainer.innerHTML = createPlaceholder('No trending songs found.');
            }
        } catch (error) {
            console.error('Failed to load trending songs:', error);
            songsContainer.innerHTML = createPlaceholder('Failed to load trending songs.');
        }
    }

    async getSpotifyTrendingTracks(forceRefresh = false) {
        const cacheTtlMs = 1000 * 60 * 20;
        const now = Date.now();

        if (
            !forceRefresh &&
            this.spotifyHomeTrendingTracksCache &&
            this.spotifyHomeTrendingTracksCache.playlistId === SPOTIFY_HOME_PLAYLIST_ID &&
            now - this.spotifyHomeTrendingTracksCache.timestamp < cacheTtlMs
        ) {
            return this.spotifyHomeTrendingTracksCache.tracks;
        }

        const seeds = await this.fetchSpotifyPlaylistSeeds();
        const uniqueSeeds = [];
        const seen = new Set();

        for (const seed of seeds) {
            const title = String(seed?.title || '').trim();
            const artists = Array.isArray(seed?.artists)
                ? seed.artists.map((name) => String(name || '').trim()).filter(Boolean)
                : [];
            if (!title || artists.length === 0) continue;

            const isrc = String(seed?.isrc || '').trim().toUpperCase();
            const key = `${title.toLowerCase()}::${artists.join(',').toLowerCase()}::${isrc}`;
            if (seen.has(key)) continue;
            seen.add(key);
            uniqueSeeds.push({ title, artists, isrc });
        }

        const matchedTracks = await Promise.all(
            uniqueSeeds.slice(0, 60).map(async (seed) => {
                try {
                    if (seed.isrc) {
                        const isrcResult = await this.api.searchTracks(`isrc:${seed.isrc}`, { limit: 5 });
                        const isrcCandidates = Array.isArray(isrcResult?.items) ? isrcResult.items : [];
                        if (isrcCandidates.length > 0) {
                            const exactIsrcMatch = isrcCandidates.find(
                                (track) => String(track?.isrc || '').toUpperCase() === seed.isrc
                            );
                            if (exactIsrcMatch) return exactIsrcMatch;
                            return isrcCandidates[0];
                        }
                    }

                    const queries = [
                        `"${seed.title}" ${seed.artists[0] || ''}`.trim(),
                        `${seed.title} ${seed.artists.join(' ')}`.trim(),
                        seed.title,
                    ];

                    for (const searchQuery of queries) {
                        if (!searchQuery) continue;
                        const result = await this.api.searchTracks(searchQuery, { limit: 20 });
                        const candidates = Array.isArray(result?.items) ? result.items : [];
                        if (candidates.length === 0) continue;

                        const bestMatch = this.pickBestSpotifyTrackMatch(
                            candidates,
                            seed.title,
                            seed.artists,
                            seed.isrc
        ...
        const rankedTracks = this.rankIndiaTrendingTracks(uniqueTracks);

        this.spotifyHomeTrendingTracksCache = {
            timestamp: now,
            playlistId: SPOTIFY_HOME_PLAYLIST_ID,
            tracks: rankedTracks,
        };
    ...
    rankIndiaTrendingTracks(tracks = []) {
        if (!Array.isArray(tracks) || tracks.length <= 1) return tracks;

        const indianArtistSet = new Set(
            HOME_POPULAR_ARTISTS.map((name) => String(name || '').trim().toLowerCase()).filter(Boolean)
        );

        return tracks
            .map((track, index) => {
                const hindiScore =
                    typeof this.api.getHindiPriorityScore === 'function' ? this.api.getHindiPriorityScore(track) : 0;
                ...
                const hasPopularIndianArtist = artistNames.some((name) => indianArtistSet.has(name));
                const score = hindiScore * 10 + (hasPopularIndianArtist ? 30 : 0);
    ...
    scrollHomeRail(containerId, direction = 'next') {
        ...
        const step = Math.max(container.clientWidth * 0.9, 280);
        const delta = direction === 'prev' ? -step : step;
        const nextScrollLeft = container.scrollLeft + delta;

        if (direction === 'prev' && nextScrollLeft <= 8) {
            container.scrollTo({ left: 0, behavior: 'smooth' });
        } else if (direction !== 'prev' && nextScrollLeft >= maxScrollLeft - 8) {
            container.scrollTo({ left: 0, behavior: 'smooth' });
        } else {
    ...
    setHomeRailLoadingState(container) {
        if (!container) return;

        container.innerHTML = `<div style="grid-column: 1/-1; padding: 0.35rem 0 1rem;">${createPlaceholder('Just a moment ,great music is on the way.')}</div>`;
    }
    ...
    async renderHomeAlbums(forceRefresh = false, providedSeeds = null, retryCount = 0) {
        ...
        if (albumsContainer) {
            albumsContainer.classList.add('home-song-rail');
            albumsContainer.classList.add('card-grid');
        }

        if (albumsContainer) {
            if (forceRefresh || albumsContainer.children.length === 0) {
                this.setHomeRailLoadingState(albumsContainer);
    ...
    async renderHomeArtists(forceRefresh = false, _providedSeeds = null) {
        ...
        if (artistsContainer) {
            artistsContainer.classList.add('home-song-rail');
            artistsContainer.classList.add('card-grid');
            ...
            try {
                const popularArtists = await this.getPopularArtists(forceRefresh);
                const filteredArtists = await this.filterUserContent(popularArtists, 'artist');

                if (filteredArtists.length > 0) {
                    artistsContainer.innerHTML = filteredArtists
                        .map((artist) =>
                            artist._popularFallback
                                ? this.createPopularArtistFallbackCardHTML(artist)
                                : this.createArtistCardHTML(artist)
                        )
                        .join('');
                    artistsContainer.scrollLeft = 0;

                    for (const artist of filteredArtists) {
                        if (artist._popularFallback) continue;
                        const el = artistsContainer.querySelector(`[data-artist-id="${artist.id}"]`);
                        if (el) {
                            trackDataStore.set(el, artist);
                            await this.updateLikeState(el, 'artist', artist.id);
                    ...
                    artistsContainer.innerHTML = createPlaceholder('No popular artists found.');
                }
            } catch (e) {
                console.error(e);
                artistsContainer.innerHTML = createPlaceholder('Failed to load popular artists.');
            }
        }
    }

    async getPopularArtists(forceRefresh = false) {
        const cacheTtlMs = 1000 * 60 * 60;
        const now = Date.now();

        if (!forceRefresh && this.popularArtistsCache && now - this.popularArtistsCache.timestamp < cacheTtlMs) {
            return this.popularArtistsCache.artists;
        }

        const resolvedArtists = await Promise.all(
            HOME_POPULAR_ARTISTS.map(async (artistName) => {
                try {
                    const result = await this.api.searchArtists(artistName, { limit: 5 });
                    const candidates = Array.isArray(result?.items) ? result.items : [];
                    const bestMatch = this.pickBestSpotifyArtistMatch(candidates, artistName);

                    if (bestMatch?.id && bestMatch?.name && bestMatch.picture) {
                        return bestMatch;
                    }
                } catch {
                    // Fall back to a local display card.
                }

                return {
                    id: `popular-${slugifyText(artistName)}`,
                    name: artistName,
                    picture: null,
                    searchQuery: artistName,
                    _popularFallback: true,
                };
            })
        );
    ...
    async getSpotifyRecommendedArtists(forceRefresh = false) {
        const cacheTtlMs = 1000 * 60 * 20;
        const now = Date.now();

        if (
            !forceRefresh &&
            this.spotifyHomeArtistsCache &&
            now - this.spotifyHomeArtistsCache.timestamp < cacheTtlMs
        ) {
            return this.spotifyHomeArtistsCache.artists;
        }
        ...
        try {
            ...
            for (const seed of seeds) {
                const artists = Array.isArray(seed?.artists) ? seed.artists : [];
            ...
            const searchResults = await Promise.all(
                uniqueNames.slice(0, 40).map(async (artistName) => {
                    try {
                        const result = await this.api.searchArtists(artistName, { limit: 5 });
                        const candidates = Array.isArray(result?.items) ? result.items : [];
                        if (candidates.length === 0) return null;

                        return this.pickBestSpotifyArtistMatch(candidates, artistName) || candidates[0];
                    } catch {
                        return null;
                    }
                })
            ...
            for (const artist of searchResults) {
                if (!artist?.id || seenArtistIds.has(artist.id)) continue;
            ...
            console.warn('Spotify artist source unavailable, using fallback recommendations.', error);
            return [];
        }
    }

    async fetchSpotifyPlaylistSeeds() {
        const uniqueSeeds = [];
        const seen = new Set();

        for (const entry of HOME_TRENDING_SONGS) {
            const seed = parseTrendingSongSeed(entry);
            if (!seed) continue;

            const key = `${seed.title.toLowerCase()}::${seed.artists.join(',').toLowerCase()}`;
            if (seen.has(key)) continue;
            seen.add(key);
    ...
    pickBestSpotifyArtistMatch(candidates, seedArtistName) {
        ...
        const seed = normalize(seedArtistName);
        let best = null;
        let bestScore = -1;
        ...
        for (const candidate of candidates) {
            ...
            if (name === seed) score += 5;
            else if (name.includes(seed) || seed.includes(name)) score += 2;
    ...
    pickBestSpotifyTrackMatch(candidates, seedTitle, seedArtists = [], seedIsrc = null) {
        if (!Array.isArray(candidates) || candidates.length === 0) return null;
        ...
        const normalizedArtists = seedArtists.map((name) => normalize(name)).filter(Boolean);
        let best = null;
        let bestScore = -1;

        for (const candidate of candidates) {
            const candidateTitle = normalize(getTrackTitle(candidate));
            const candidateArtists = (Array.isArray(candidate?.artists) ? candidate.artists : [])
                .map((artist) => normalize(artist?.name || artist))
                .filter(Boolean);
            const candidateIsrc = String(candidate?.isrc || '').toUpperCase();
            let score = 0;

            if (seedIsrc && candidateIsrc && candidateIsrc === seedIsrc) score += 10;

            if (candidateTitle === normalizedTitle) score += 5;
            else if (candidateTitle.includes(normalizedTitle) || normalizedTitle.includes(candidateTitle)) score += 2;

            if (normalizedArtists.length > 0 && candidateArtists.length > 0) {
                if (candidateArtists[0] === normalizedArtists[0]) score += 4;

                const overlap = candidateArtists.filter((artist) => normalizedArtists.includes(artist)).length;
                score += overlap * 2;
            }
    ...
    async renderHomeRecent() {
        ...
        if (recentContainer) {
            recentContainer.classList.add('home-song-rail');
            recentContainer.classList.add('card-grid');
    ...
    async renderSearchPage(query) {
        ...
        try {
            ...
            finalTracks = await this.ensureCuratedTopSearchTracks(query, finalTracks, signal);
            finalTracks = this.prioritizeCuratedTopSearchTracks(query, finalTracks);
    ...
    renderSearchHistory() {
        ...
        if (!historyEl) return;
        const rawHistory = JSON.parse(localStorage.getItem('search-history') || '[]');
        const history = rawHistory
            .map((entry) => {
                if (typeof entry === 'string') {
                    return { query: entry, cover: null };
                }
                if (entry && typeof entry === 'object' && typeof entry.query === 'string') {
                    return { query: entry.query, cover: entry.cover || null };
                }
                return null;
            })
            .filter(Boolean);
        ...
        historyEl.innerHTML =
            history
                .map(
                    (entry) => `
            <div class="search-history-item" data-query="${escapeHtml(entry.query)}">
                ${
                    entry.cover
                        ? this.getCoverHTML(entry.cover, escapeHtml(entry.query), 'search-history-thumb', 'eager')
                        : `<img src="/icon.jpg" class="search-history-thumb" alt="${escapeHtml(entry.query)}" loading="eager">`
                }
                <span class="query-text">${escapeHtml(entry.query)}</span>
    ...
    removeFromSearchHistory(query) {
        ...
        history = history.filter((entry) => {
            if (typeof entry === 'string') return entry !== query;
            if (entry && typeof entry === 'object') return entry.query !== query;
            return false;
        });
    ...
    addToSearchHistory(query, cover = null) {
        if (!query || query.trim().length === 0) return;
        const normalizedQuery = query.trim();
        const inferredCover =
            cover ||
            this.searchSuggestionCoverByQuery.get(normalizedQuery.toLowerCase()) ||
            this.searchSuggestionCoverByQuery.get(normalizedQuery) ||
            null;
        let history = JSON.parse(localStorage.getItem('search-history') || '[]')
            .map((entry) => {
                if (typeof entry === 'string') return { query: entry, cover: null };
                if (entry && typeof entry === 'object' && typeof entry.query === 'string') {
                    return { query: entry.query, cover: entry.cover || null };
                }
                return null;
        ...
        const existingEntry = history.find((entry) => entry.query === normalizedQuery);
        const existingCover = existingEntry?.cover || null;

        history = history.filter((entry) => entry.query !== normalizedQuery);
        history.unshift({ query: normalizedQuery, cover: inferredCover || existingCover || null });
        history = history.slice(0, 10);

        localStorage.setItem('search-history', JSON.stringify(history));
    }

    normalizeSearchComparableText(value) {
        return String(value || '')
            .toLowerCase()
            .normalize('NFKC')
            .replace(/[^a-z0-9\u0900-\u097f]+/g, ' ')
    ...
    getCuratedTrendingSearchSeeds() {
        if (this.curatedTrendingSearchSeedsCache) return this.curatedTrendingSearchSeedsCache;

        this.curatedTrendingSearchSeedsCache = TOP_TRENDING_SEARCH_SEEDS.map(([title, artistList]) => {
            const titleNorm = this.normalizeSearchComparableText(title);
            const artistNorms = String(artistList || '')
                .split(';')
                .map((artist) => this.normalizeSearchComparableText(artist))
                .filter(Boolean);
            return {
                title,
                titleNorm,
                artists: String(artistList || '')
                    .split(';')
                    .map((artist) => artist.trim())
                    .filter(Boolean),
                artistNorms,
        ...
        return this.curatedTrendingSearchSeedsCache;
    }

    getMatchingCuratedTrendingSeeds(query) {
        const normalizedQuery = this.normalizeSearchComparableText(query);
        if (!normalizedQuery) return [];

        return this.getCuratedTrendingSearchSeeds().filter((seed) => {
            if (!seed.titleNorm) return false;
            return (
                seed.titleNorm === normalizedQuery ||
                seed.titleNorm.includes(normalizedQuery) ||
                normalizedQuery.includes(seed.titleNorm)
            );
        });
    }

    getTrackArtistNorms(track) {
        return [track?.artist?.name, ...(Array.isArray(track?.artists) ? track.artists.map((artist) => artist?.name) : [])]
            .map((name) => this.normalizeSearchComparableText(name))
            .filter(Boolean);
    }

    doesTrackMatchCuratedSeed(track, seed) {
        const trackTitleNorm = this.normalizeSearchComparableText(getTrackTitle(track) || track?.title || '');
        if (!trackTitleNorm || !seed?.titleNorm) return false;

        const titleMatch =
            trackTitleNorm === seed.titleNorm ||
            trackTitleNorm.includes(seed.titleNorm) ||
            seed.titleNorm.includes(trackTitleNorm);

        if (!titleMatch) return false;

        const artistNorms = this.getTrackArtistNorms(track);
        if (!seed.artistNorms.length) return true;

        return artistNorms.some((artist) => seed.artistNorms.includes(artist));
    }

    prioritizeCuratedTopSearchTracks(query, tracks = []) {
        if (!Array.isArray(tracks) || tracks.length <= 1) return tracks;

        const normalizedQuery = this.normalizeSearchComparableText(query);
        const matchingSeeds = this.getMatchingCuratedTrendingSeeds(normalizedQuery);
        if (!matchingSeeds.length) return tracks;
        ...
        return tracks
            .map((track, index) => {
                ...
                for (const seed of matchingSeeds) {
                    if (this.doesTrackMatchCuratedSeed(track, seed)) {
                        score += 1000;
                        if (trackTitleNorm === seed.titleNorm) score += 300;
                    }
                }

                if (trackTitleNorm === normalizedQuery) score += 200;
                else if (trackTitleNorm.startsWith(normalizedQuery)) score += 120;
                else if (trackTitleNorm.includes(normalizedQuery)) score += 80;
    ...
    async ensureCuratedTopSearchTracks(query, tracks = [], signal) {
        if (!Array.isArray(tracks)) return [];

        const normalizedQuery = this.normalizeSearchComparableText(query);
        const exactSeeds = this.getMatchingCuratedTrendingSeeds(normalizedQuery).filter(
            (seed) => seed.titleNorm === normalizedQuery
        );
        if (!exactSeeds.length) return tracks;

        const result = [...tracks];
        const existingIds = new Set(result.map((track) => track?.id).filter(Boolean));

        for (const seed of exactSeeds.slice(0, 3)) {
            const alreadyPresent = result.some((track) => this.doesTrackMatchCuratedSeed(track, seed));
            if (alreadyPresent) continue;

            try {
                const targetedQuery = `${seed.title} ${seed.artists[0] || ''}`.trim();
                const targeted = await this.api.searchTracks(targetedQuery, { signal, limit: 20 });
                if (signal?.aborted) return result;

                const candidate = (targeted.items || []).find((track) => this.doesTrackMatchCuratedSeed(track, seed));
                if (!candidate?.id || existingIds.has(candidate.id)) continue;

                result.unshift(candidate);
                existingIds.add(candidate.id);
            } catch {
                // Ignore targeted fetch failures and keep default results.
            }
        }
    ...
    async renderSearchSuggestions(query) {
        ...
        if (this.searchSuggestionAbortController) {
            this.searchSuggestionAbortController.abort();
        }
        this.searchSuggestionAbortController = new AbortController();
        const signal = this.searchSuggestionAbortController.signal;

        historyEl.innerHTML = `
            <div class="search-history-item search-suggestion-item search-suggestion-status" aria-disabled="true">
                <span class="query-text">Suggesting songs...</span>
            </div>
        `;
        historyEl.style.display = 'block';

        try {
            const results = await this.api.searchTracks(normalizedQuery, { signal });
            if (signal.aborted) return;

            const baseSuggestionPool = (results.items || []).filter((track) => track?.id && track?.title);
            const suggestionPool = await this.ensureCuratedTopSearchTracks(normalizedQuery, baseSuggestionPool, signal);
            if (signal.aborted) return;
            const suggestions = this.prioritizeCuratedTopSearchTracks(normalizedQuery, suggestionPool).slice(0, 6);
            this.searchSuggestionCoverByQuery.clear();
            suggestions.forEach((track) => {
                if (!track?.title) return;
                const key = String(track.title).trim().toLowerCase();
                if (!key || !track.album?.cover) return;
                this.searchSuggestionCoverByQuery.set(key, track.album.cover);
            });
            if (suggestions.length > 0 && suggestions[0].album?.cover) {
                this.searchSuggestionCoverByQuery.set(normalizedQuery.toLowerCase(), suggestions[0].album.cover);
            }

            if (suggestions.length === 0) {
                historyEl.innerHTML = `
                    <div class="search-history-item search-suggestion-item search-suggestion-status" aria-disabled="true">
                        <span class="query-text">No song suggestions yet.</span>
                    </div>
                `;
                historyEl.style.display = 'block';
                return;
            ...
            historyEl.innerHTML = suggestions
                .map((track) => {
                    const artistText = getTrackArtists(track) || 'Unknown Artist';
                    const title = escapeHtml(track.title);
                    const artist = escapeHtml(artistText);
                    const titleRaw = escapeHtml(track.title);
                    const cover = track.album?.cover || '';
                    const trackId = escapeHtml(String(track.id));
                    return `
                        <div class="search-history-item search-suggestion-item" data-query="${titleRaw}" data-cover="${escapeHtml(String(cover))}" data-track-id="${trackId}">
                            ${this.getCoverHTML(cover, titleRaw, 'search-history-thumb', 'eager')}
                            <span class="query-text">
                                <span class="suggestion-title">${title}</span>
                                <span class="suggestion-subtext">${artist}</span>
                            </span>
                        </div>
                    `;
                })
            ...
            const suggestionById = new Map(suggestions.map((track) => [String(track.id), track]));
            historyEl.querySelectorAll('.search-suggestion-item[data-query]').forEach((item) => {
                item.addEventListener('click', () => {
                    const selectedQuery = decodeHtml(item.dataset.query || '');
                    const selectedCover = item.dataset.cover || null;
                    const selectedTrackId = item.dataset.trackId || '';
                    const selectedTrack = suggestionById.get(String(selectedTrackId));
                    const searchInput = document.getElementById('search-input');
                    if (!searchInput) return;

                    searchInput.value = selectedQuery;
                    this.addToSearchHistory(selectedQuery, selectedCover);

                    if (selectedTrack?.type === 'video') {
                        this.player.playVideo(selectedTrack);
                    } else if (selectedTrack) {
                        this.player.setQueue([selectedTrack], 0);
    ...
    async renderAlbumPage(albumId, provider = null) {
        ...
        try {
            const customAlbumData = isCustomAlbumId(albumId) ? await resolveCustomAlbum(this.api, albumId) : null;
            const { album, tracks } = customAlbumData || (await this.api.getAlbum(albumId, provider));
            ...
            const fallbackCoverId = tracks[0]?.album?.cover || tracks[0]?.cover || null;
            const coverUrl =
                videoCoverUrl ||
                (album.cover ? this.api.getCoverUrl(album.cover) : fallbackCoverId ? this.api.getCoverUrl(fallbackCoverId) : '');
            ...
            this.setPageBackground(coverUrl || null);
            ...
            if (album._isCustomAlbum) {
                prodEl.textContent = `By ${album.artist.name}` + (firstCopyright ? ` • ${firstCopyright}` : '');
            } else {
                prodEl.innerHTML =
    ...
    async renderPlaylistPage(playlistId, source = null, _provider = null) {
        ...
        try {
            ...
            if (playlistData) {
                ...
                document.title = `${playlistData.name || playlistData.title} - audivo.in`;
    ...
    async renderTrackPage(trackId, provider = null) {
        ...
        const badgesEl = document.getElementById('track-detail-badges');
        const albumLinkEl = document.getElementById('track-detail-album-link');
        ...
        if (albumLinkEl) albumLinkEl.innerHTML = '';
        ...
        try {
            ...
            if (badgesEl) {
                const badges = [];
                if (track.duration) badges.push(`<span class="track-detail-badge">${Math.floor(track.duration / 60)}:${String(track.duration % 60).padStart(2, '0')}</span>`);
                if (track.explicit) badges.push('<span class="track-detail-badge">Explicit</span>');
                if (track.audioQuality) badges.push(`<span class="track-detail-badge">${escapeHtml(String(track.audioQuality))}</span>`);
            ...
            if (track.album) {
                ...
                if (albumLinkEl) {
                    albumLinkEl.innerHTML = `<a class="track-detail-album-chip" href="/album/${track.album.id}">View album</a>`;
                }
            }
    ...
    async renderPodcastsBrowsePage() {
        ...
        document.title = 'Podcasts - audivo.in';
    }
    ...
    async renderPodcastPage(podcastId) {
        ...
        try {
            ...
            document.title = `${podcastResult?.title || 'Podcast'} - audivo.in`;
