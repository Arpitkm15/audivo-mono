...
export class LosslessAPI {
    ...
    getHindiPriorityScore(item) {
        if (!item || typeof item !== 'object') return 0;

        let score = 0;
        const devanagariRegex = /[\u0900-\u097F]/;
        const hindiKeywords = ['hindi', 'bollywood'];
        ...
        const textFields = [
            ...
            item.album?.title,
            item.artist?.name,
            ...(Array.isArray(item.artists) ? item.artists.map((a) => a?.name) : []),
        ]
            .filter((value) => typeof value === 'string')
            .join(' ');

        const normalizedText = textFields.toLowerCase();
        if (devanagariRegex.test(textFields)) {
            score += 4;
        }

        if (hindiKeywords.some((keyword) => normalizedText.includes(keyword))) {
            score += 3;
        }
        ...
        const hasHindiLanguageTag = languageCandidates.some((value) => {
            const normalized = value.toLowerCase().trim();
            return normalized === 'hi' || normalized.startsWith('hi-') || normalized.includes('hindi');
        });

        if (hasHindiLanguageTag) {
            score += 5;
        }
    ...
    prioritizeHindiItems(items = []) {
        if (!Array.isArray(items) || items.length <= 1) return items;

        return items
            .map((item, index) => ({ item, index, score: this.getHindiPriorityScore(item) }))
