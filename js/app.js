...
async function getAlbumDataForRoute(albumId) {
    if (isCustomAlbumId(albumId)) {
        const customAlbum = await resolveCustomAlbum(MusicAPI.instance, albumId);
        if (!customAlbum) {
            throw new Error('Custom album could not be loaded');
        }
        return customAlbum;
    }

    return MusicAPI.instance.getAlbum(albumId);
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.addEventListener('click', async (e) => {
        if (e.target.closest('#play-album-btn')) {
            ...
            try {
                const { tracks } = await getAlbumDataForRoute(albumId);
        ...
        if (e.target.closest('#download-album-btn')) {
            ...
            try {
                const { album, tracks } = await getAlbumDataForRoute(albumId);
    ...
    const handleRouteChange = async (event) => {
        ...
        // Hide account control across all pages.
        const headerAccountControl = document.querySelector('.header-account-control');
        if (headerAccountControl) {
            headerAccountControl.style.display = 'none';
        }
...
function showUpdateNotification(updateCallback) {
    ...
    notification.innerHTML = `
        <div>
            ...
            <p>A new version of audivo.in is available.</p>
        </div>
